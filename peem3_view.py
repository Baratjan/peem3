...
if uploaded_file is not None:
    ...
    # Define 1024x1024 Crop parameters
    target = 1024
    ...
    with tab_view:
        ...
        c1, col_img, c3 = st.columns([1, 2, 1])
        with col_img:
    ...
    with tab_drift:
        st.header("Robust Drift Correction (Pre-cropped 1024x1024)")
        
        # Immediate Crop for Drift Logic
        # This creates a 1024x1024 view of the data to work with
        working_stack = mmap_stack[:, sh:sh+target, sw:sw+target]
        ...
        with col_ctrl:
            ...
            # Coordinates are now relative to the 1024x1024 crop
            roi_x = st.slider("ROI Center X", 0, 1024, 512)
            roi_y = st.slider("ROI Center Y", 0, 1024, 512)
            
            run_drift = st.button("🚀 Run Alignment on Cropped Stack", use_container_width=True)

        with col_prev:
            y1, y2 = max(0, roi_y - roi_size//2), min(1024, roi_y + roi_size//2)
            x1, x2 = max(0, roi_x - roi_size//2), min(1024, roi_x + roi_size//2)
            
            ref_frame = working_stack[0].copy()
            ...
            st.image(preview_rgb, caption="Tracking ROI (Red Box) within 1024x1024 crop", use_container_width=True)

        if run_drift:
            with st.spinner("Aligning 1024x1024 stack..."):
                cum_y, cum_x = [0.0], [0.0]
                window = filters.window('hann', (y2-y1, x2-x1))
                
                # Apply Guyader filter for robust gradient detection
                ref_roi = apply_guyader_filter(working_stack[0, y1:y2, x1:x2], sigma_val) * window
                
                for i in range(1, n_frames):
                    mov_roi = apply_guyader_filter(working_stack[i, y1:y2, x1:x2], sigma_val) * window
                    # Peak displacement identification
                ...
                # Apply Bi-Cubic (Order 3) correction to the cropped stack
                corrected = np.zeros_like(working_stack, dtype=np.float32)
                for i in range(n_frames):
                    corrected[i] = shift(working_stack[i], shift=(cum_y[i], cum_x[i]), order=3, mode='constant', cval=0)

                # Store for export
                st.session_state.aligned_cropped = corrected
                st.session_state.cum_x, st.session_state.cum_y = cum_x, cum_y
                st.success("Alignment Complete!")

        if 'aligned_cropped' in st.session_state:
            st.markdown("---")
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".tif") as out_tmp:
                ...
                tiff.imwrite(out_tmp.name, st.session_state.aligned_cropped, photometric='minisblack')
                with open(out_tmp.name, "rb") as f:
                    st.download_button("💾 Download Aligned 1024x1024 Stack", f, "aligned_1024_stack.tif")

    # --- TAB 3: FFT MASKING ---
    with tab_fft:
        st.header("Interactive FFT Spot-Masking")
        f_idx = st.slider("Select Frame", 0, n_frames - 1, 0, key="fft_frame")
        
        # Analyze the cropped 1024 version
        img_raw = mmap_stack[f_idx, sh:sh+target, sw:sw+target].astype(np.float64)
        ...
        with col_fft:
            ...
            coords = st.text_input("Coordinates (x,y ; x,y)", "512,512")
        ...
        with col_recon:
            ...
            try:
                for coord in coords.split(';'):
                    ...
                    # Friedel conjugate for symmetry
                    cv2.circle(mask, (target - cx, target - cy), radius, fill_val, -1)
            except:
                st.warning("Format: x,y ; x,y")

            recon = np.abs(np.fft.ifft2(np.fft.ifftshift(dft_shift * mask)))
            st.image(recon / (recon.max() + 1e-8), caption="Reconstructed 1024x1024 Image", use_container_width=True)
